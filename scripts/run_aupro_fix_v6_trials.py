...
def post_overrides(*, foreground: bool = False, norm: str = "none", smooth: int = 0) -> dict[str, object]:
    return {
        "data.mask_resize_mode": "max",
...
def proto_overrides(
    *,
    ...
    return {
        ...
        "train.batch_size": 1 if size >= 448 else 2,
        ...
        **post_overrides(foreground=foreground, norm=norm, smooth=smooth),
    }
...
def can_trials() -> list[Trial]:
    return [
        Trial("maskmax_proto448_bicubic_aupro_nomask_tk005", proto_overrides(size=448, mode="bicubic", monitor="au_pro", topk=0.005, mask=False)),
        Trial("maskmax_proto448_bicubic_pro_nomask_tk20", proto_overrides(size=448, mode="bicubic", monitor="selection_score_pro", topk=0.2, mask=False)),
        Trial("maskmax_proto560_bicubic_aupro_nomask_tk005", proto_overrides(size=560, mode="bicubic", monitor="au_pro", topk=0.005, mask=False)),
        Trial("maskmax_proto672_nearest_aupro_nomask_tk002", proto_overrides(size=672, mode="nearest", monitor="au_pro", topk=0.002, mask=False)),
        Trial("maskmax_score01_proto448_bicubic_aupro_nomask", score_overrides(alpha=0.01, size=448, mode="bicubic", monitor="au_pro", topk=0.005, mask=False)),
        Trial("maskmax_score02_proto448_bicubic_aupro_nomask", score_overrides(alpha=0.02, size=448, mode="bicubic", monitor="au_pro", topk=0.005, mask=False)),
    ]


def wallplugs_trials() -> list[Trial]:
    return [
        Trial("maskmax_proto560_bilinear_fg_minmax_s3_pro_mask", proto_overrides(size=560, mode="bilinear", monitor="selection_score_pro", topk=0.05, mask=True, foreground=True, norm="image_minmax", smooth=3)),
        Trial("maskmax_proto560_bilinear_fg_minmax_s5_pro_mask", proto_overrides(size=560, mode="bilinear", monitor="selection_score_pro", topk=0.05, mask=True, foreground=True, norm="image_minmax", smooth=5)),
        Trial("maskmax_proto560_nearest_fg_pro_mask", proto_overrides(size=560, mode="nearest", monitor="selection_score_pro", topk=0.005, mask=True, foreground=True)),
        Trial("maskmax_proto448_bicubic_fg_minmax_s3_aupro_nomask", proto_overrides(size=448, mode="bicubic", monitor="au_pro", topk=0.005, mask=False, foreground=True, norm="image_minmax", smooth=3)),
        Trial("maskmax_score01_proto560_bilinear_fg_minmax_s3_mask", score_overrides(alpha=0.01, size=560, mode="bilinear", monitor="selection_score_pro", topk=0.05, mask=True, foreground=True, norm="image_minmax", smooth=3)),
    ]


def pushpins_trials() -> list[Trial]:
    return [
        Trial("maskmax_proto560_bilinear_pro_mask", proto_overrides(size=560, mode="bilinear", monitor="selection_score_pro", topk=0.05, mask=True)),
        Trial("maskmax_proto560_bicubic_aupro_mask_tk20", proto_overrides(size=560, mode="bicubic", monitor="au_pro", topk=0.2, mask=True)),
        Trial("maskmax_proto672_bicubic_aupro_mask_tk005", proto_overrides(size=672, mode="bicubic", monitor="au_pro", topk=0.005, mask=True)),
        Trial("maskmax_proto672_nearest_pixelap_mask_tk005", proto_overrides(size=672, mode="nearest", monitor="pixel_ap", topk=0.005, mask=True)),
        Trial("maskmax_score01_proto560_bicubic_aupro_nomask", score_overrides(alpha=0.01, size=560, mode="bicubic", monitor="au_pro", topk=0.005, mask=False)),
    ]
...
def screw_bag_trials() -> list[Trial]:
    return [
        Trial("maskmax_proto448_bicubic_aupro_nomask_tk005", proto_overrides(size=448, mode="bicubic", monitor="au_pro", topk=0.005, mask=False)),
        Trial("maskmax_proto448_bicubic_fg_aupro_nomask_tk005", proto_overrides(size=448, mode="bicubic", monitor="au_pro", topk=0.005, mask=False, foreground=True)),
        Trial("maskmax_proto560_bilinear_fg_pro_mask", proto_overrides(size=560, mode="bilinear", monitor="selection_score_pro", topk=0.05, mask=True, foreground=True)),
        Trial("maskmax_proto560_bilinear_pro_mask", proto_overrides(size=560, mode="bilinear", monitor="selection_score_pro", topk=0.05, mask=True)),
        Trial("maskmax_score01_proto448_bicubic_fg_aupro_nomask", score_overrides(alpha=0.01, size=448, mode="bicubic", monitor="au_pro", topk=0.005, mask=False, foreground=True)),
    ]


def v6_trials(dataset: str, category: str) -> list[Trial]:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", default="outputs/aupro_fix_v6")
    parser.add_argument("--log-root", default="outputs/logs/aupro_fix_v6_trials")
...
def build_tasks(args: argparse.Namespace) -> list[Task]:
    ...
    for experiment in experiments:
        for trial in v6_trials(experiment.dataset, experiment.category):
    ...
    if not tasks:
        raise RuntimeError(f"No v6 tasks matched targets={sorted(targets)}")
    return tasks
...
def launch(args: argparse.Namespace, task: Task) -> tuple[subprocess.Popen, object]:
    ...
    print(f"[aupro-v6] START {task.name} log={task.log_file}", flush=True)
...
def summarize(tasks: list[Task], output_root: Path) -> None:
    ...
    print(f"[aupro-v6] wrote {summary_path}", flush=True)
    print(f"[aupro-v6] wrote {best_path}", flush=True)
...
def run_tasks(args: argparse.Namespace) -> None:
    ...
    print(
        f"[aupro-v6] total={len(tasks)} skipped={len(tasks) - len(pending)} "
    ...
    while pending or running:
        while pending and len(running) < max(1, int(args.max_parallel)):
            ...
            if completed(task.result_file) or (args.skip_running and is_running(task)):
                print(f"[aupro-v6] SKIP {task.name}", flush=True)
                continue
        ...
        for task, process, handle in running:
            ...
            if code != 0:
                ...
                if attempt < max(0, int(args.retries)):
                    ...
                    print(f"[aupro-v6] RETRY {task.name} exit={code}", flush=True)
                    pending.insert(0, task)
                    continue
                print(f"[aupro-v6] FAIL {task.name} exit={code} log={task.log_file}", flush=True)
                raise SystemExit(code)
            print(f"[aupro-v6] DONE {task.name}", flush=True)
