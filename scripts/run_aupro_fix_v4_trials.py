...
def v4_trials() -> list[Trial]:
    proto = {"energy.alpha": 0.0, "energy.beta": 1.0, "energy.gamma": 0.0, "energy.eta": 0.0}
    score_proto = {"energy.alpha": 0.05, "energy.beta": 1.0, "energy.gamma": 0.0, "energy.eta": 0.0}
    return [
        Trial(
            "proto560_bicubic_aupro_no_mask",
            {
                **proto,
                "data.image_size": 560,
                "train.batch_size": 1,
        ...
        Trial(
            "proto560_bilinear_aupro_no_mask",
            {
                **proto,
                "data.image_size": 560,
                "train.batch_size": 1,
        ...
        Trial(
            "score_proto560_bicubic_aupro_no_mask",
            {
                **score_proto,
                "data.image_size": 560,
                "train.batch_size": 1,
        ...
        Trial(
            "proto672_bicubic_aupro_no_mask",
            {
                **proto,
                "data.image_size": 672,
                "train.batch_size": 1,
        ...
        Trial(
            "proto448_bicubic_aupro_no_mask_topk01",
            {
                **proto,
                "data.image_size": 448,
                "train.batch_size": 1,
        ...
        Trial(
            "proto448_bicubic_aupro_no_mask_topk10",
            {
                **proto,
                "data.image_size": 448,
                "train.batch_size": 1,
                "energy.topk_ratio": 0.1,
                "energy.upsample_mode": "bicubic",
                "train.early_stopping.monitor": "au_pro",
                "graph.use_mask_topology": "false",
                "graph.beta_m": 0.0,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", default="outputs/aupro_fix_v4")
    parser.add_argument("--log-root", default="outputs/logs/aupro_fix_v4_trials")
...
def build_tasks(args: argparse.Namespace) -> list[Task]:
    ...
    experiments = [experiment for experiment in downloaded_experiments() if experiment.dataset in selected]
    if args.category:
        experiments = [experiment for experiment in experiments if experiment.category == args.category]
    tasks: list[Task] = []
    for experiment in experiments:
        for trial in v4_trials():
    ...
    if not tasks:
        raise RuntimeError(f"No v4 tasks matched datasets={sorted(selected)} category={args.category!r}")
    return tasks
...
def launch(args: argparse.Namespace, task: Task) -> tuple[subprocess.Popen, object]:
    ...
    print(f"[aupro-v4] START {task.name} log={task.log_file}", flush=True)
...
def run_tasks(args: argparse.Namespace) -> None:
    ...
    print(f"[aupro-v4] total={len(tasks)} skipped={len(tasks)-len(pending)} pending={len(pending)} max_parallel={args.max_parallel}", flush=True)
    if args.dry_run:
        for task in pending:
    ...
    while pending or running:
        while pending and len(running) < max(1, int(args.max_parallel)):
            ...
            if completed(task.result_file) or (args.skip_running and is_running(task)):
                print(f"[aupro-v4] SKIP {task.name}", flush=True)
                continue
        ...
        for task, process, handle in running:
            ...
            if code != 0:
                ...
                if attempt < max(0, int(args.retries)):
                    ...
                    print(f"[aupro-v4] RETRY {task.name} exit={code}", flush=True)
                    pending.insert(0, task)
                    continue
                print(f"[aupro-v4] FAIL {task.name} exit={code} log={task.log_file}", flush=True)
                raise SystemExit(code)
            print(f"[aupro-v4] DONE {task.name}", flush=True)
