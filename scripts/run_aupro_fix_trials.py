...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", default="outputs/aupro_fix")
    parser.add_argument("--log-root", default="outputs/logs/aupro_fix_trials")
...
def trials() -> list[Trial]:
    ...
    score_proto = {"energy.alpha": 0.1, "energy.beta": 1.0, "energy.gamma": 0.0, "energy.eta": 0.0}
    return [
        Trial(
            "proto336_bicubic_pro",
            {
                **proto,
                "data.image_size": 336,
                "train.batch_size": 2,
        ...
        Trial(
            "proto_topo1e-4_336_bicubic_pro",
            {
                "energy.alpha": 0.0,
                "energy.beta": 1.0,
                "energy.gamma": 0.0001,
        ...
        Trial(
            "proto336_bicubic_aupro_no_mask",
            {
                **proto,
                "data.image_size": 336,
                "train.batch_size": 2,
        ...
        Trial(
            "proto448_bicubic_aupro_no_mask",
            {
                **proto,
                "data.image_size": 448,
                "train.batch_size": 1,
        ...
        Trial(
            "score_proto336_bicubic_aupro_no_mask",
            {
                **score_proto,
                "data.image_size": 336,
                "train.batch_size": 2,
        ...
        Trial(
            "score_proto448_bicubic_aupro_no_mask",
            {
                **score_proto,
                "data.image_size": 448,
                "train.batch_size": 1,
...
def is_running(task: Task, command_text: str | None = None) -> bool:
    ...
    return f"eval.result_dir={task.result_dir}" in command_text or f"train.save_dir={task.checkpoint_dir}" in command_text
...
def experiments_for(args: argparse.Namespace) -> list[Experiment]:
    ...
    if not experiments:
        raise RuntimeError(f"No experiments found for dataset={args.dataset!r} category={args.category!r}.")
    return experiments
...
def launch(task: Task, args: argparse.Namespace) -> tuple[subprocess.Popen, object]:
    ...
    print(f"[aupro-fix] START {task.name} log={task.log_file}", flush=True)
...
def run_tasks(args: argparse.Namespace) -> None:
    ...
    print(
        f"[aupro-fix] total={len(tasks)} skipped={len(tasks) - len(pending)} "
    ...
    while pending or running:
        while pending and len(running) < max(1, int(args.max_parallel)):
            ...
            if completed(task.result_file) or (args.skip_running and is_running(task)):
                print(f"[aupro-fix] SKIP {task.name}", flush=True)
                continue
        ...
        for task, process, handle in running:
            ...
            if code != 0:
                ...
                if attempt < max(0, int(args.retries)):
                    ...
                    print(f"[aupro-fix] RETRY {task.name} exit={code}", flush=True)
                    pending.insert(0, task)
                    continue
                print(f"[aupro-fix] FAIL {task.name} exit={code} log={task.log_file}", flush=True)
                raise SystemExit(code)
            print(f"[aupro-fix] DONE {task.name}", flush=True)
